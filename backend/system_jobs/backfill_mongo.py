...
async def run(*, dry_run: bool = False) -> int:
    ...
    mdb = mongo[os.environ.get("DB_NAME", "gb_construction")]
    ...
    async with db_pg.system_conn() as conn:
        ...
        # Qui si mappano solo membership se user_id già presente in env map,
        # e si stampano gli inviti di reset.
        invites = []
        for u in users:
            ...
            try:
                await conn.execute(
                    ...
                    insert into public.leads (
                      ...
                      coalesce($8, false), coalesce($9, 'nuovo'), $10, coalesce($11, '{}'),
                    ...
                    lead.get("tags") or [],
                    lead.get("score"),
                    __import__("json").dumps(lead.get("config") or {}),
                    __import__("json").dumps(lead.get("stima")) if lead.get("stima") else None,
                    __import__("json").dumps(lead.get("tracking") or {}),
                    __import__("json").dumps(lead.get("timeline") or []),
        ...
        for c in cantieri:
            ...
            try:
                await conn.execute(
                    ...
                    c.get("milestone"),
                    c.get("note"),
                    __import__("json").dumps(c.get("fasi") or []),
                    mid,
                )
                cant_ok += 1
            except Exception as exc:
                print(f"  cantiere fail {mid}: {exc}")
        ...
        # non fail se Postgres ha più righe (seed + backfill)
