    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="bg-brand text-white rounded-full px-5 py-2 font-display uppercase text-xs tracking-wider hover:scale-105 transition-transform"
        >
          + Nuovo preventivo
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-sm">
            ...
            <tbody>
              {list.map((p) => (
                <tr
                  key={`${p.source || "legacy"}-${p.id}`}
                  className="border-b border-stroke/60 hover:bg-surface-2/50 cursor-pointer"
                  onClick={() => openPreventivo(p)}
                  role="link"
                  tabIndex={0}
                  onKeyDown={(event) => {
                    if (
                ...
                >
                  <td className="px-4 py-3 font-display uppercase text-xs text-ink">
                    {p.cliente}
                  </td>
                  ...
                  <td className="px-4 py-3 font-display text-xs text-brand">
                    {formatEuro(p.range_basso)}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`font-display uppercase text-[10px] px-2 py-1 rounded-full ${STATI[p.status]?.bg} ${STATI[p.status]?.color}`}
                    >
                      {STATI[p.status]?.label}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    {[
                      ...
                      <span
                        className={`font-display text-[10px] px-2 py-0.5 rounded-full ${p.giorni_silenzio > 10 ? "bg-red-500/20 text-red-400" : p.giorni_silenzio >= 5 ? "bg-yellow-500/20 text-yellow-400" : "bg-emerald-500/20 text-emerald-400"}`}
                      >
                        {p.giorni_silenzio}gg
                      </span>
                    ) : (
                  ...
                  >
                    <div className="flex items-center justify-end gap-2 text-fog">
                      <button
                        ...
                        aria-label={
                          p.source === "edilos"
                            ? `Scarica PDF ${p.numero || p.cliente}`
                            : `Apri scheda ${p.cliente}`
                        }
                        className="hover:text-ink"
                        title={
                          p.source === "edilos" ? "Scarica PDF" : "Apri scheda"
                        }
                      >
                      ...
                      {buildWhatsappUrl(p.telefono, p.cliente) ? (
                        <a
                          href={buildWhatsappUrl(p.telefono, p.cliente)}
                          target="_blank"
                          rel="noreferrer"
                          aria-label={`Scrivi a ${p.cliente} su WhatsApp`}
