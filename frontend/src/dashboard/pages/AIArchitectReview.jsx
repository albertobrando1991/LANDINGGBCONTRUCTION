...
const STATUS = {
  ...
  needs_confirmation: { label: "Da confermare", cls: "bg-warning/15 text-warning" },
...
export default function AIArchitectReview() {
  ...
  const { data: jobs = [], isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await client.get("/ai-architect/jobs", { params: { status: tab, q: q || undefined } })).data,
  });

  useEffect(() => {
    if (!selectedId && jobs.length > 0) setSelectedId(jobs[0].id);
    if (selectedId && jobs.length > 0 && !jobs.some((job) => job.id === selectedId)) setSelectedId(jobs[0].id);
  }, [jobs, selectedId]);

  const { data: selectedJob, isFetching: loadingJob } = useQuery({
    queryKey: ["ai-architect-job", selectedId],
    enabled: Boolean(selectedId),
    queryFn: async () => (await client.get(`/ai-architect/jobs/${selectedId}`)).data,
  });
  ...
  const approve = useMutation({
    ...
    onError: (err) => toast.error(formatApiErrorDetail(err.response?.data?.detail)),
  });
  ...
  const analysisJson = analysis?.json_content || selectedJob?.vision_analysis || {};
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <h1 className="font-display font-bold uppercase text-3xl text-ink">Revisione concept</h1>
          <p className="font-body text-sm text-fog mt-2 max-w-2xl">
            Approva la proposta 2D prima di generare render e report finali. I costi AI partono solo dopo validazione.
          </p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            ...
            <div key={label} className="rounded-xl border border-stroke bg-surface px-4 py-3">
        ...
        <section className="lg:w-[430px] space-y-4">
            ...
            {TABS.map((item) => (
              <button
                ...
                className={`font-display uppercase text-xs tracking-wider px-4 py-2 rounded-full border transition-colors ${
                  tab === item.key ? "bg-brand text-white border-brand" : "bg-surface text-fog border-stroke hover:text-ink"
                }`}
              >
                {item.label}
              </button>
          ...
          <div className="bg-surface border border-stroke rounded-2xl overflow-hidden">
            {isLoading ? (
              ...
              <div className="px-4 py-10 text-center font-body text-sm text-fog">Nessun job in questa vista.</div>
            ) : (
              <div className="divide-y divide-stroke/70">
                {jobs.map((job) => {
                  const meta = STATUS[job.status] || STATUS.queued;
                  const confidence = job.vision_analysis?.confidence ?? job.plan_type_confidence;
                  return (
                    <button
                      key={job.id}
                      onClick={() => setSelectedId(job.id)}
                      className={`w-full text-left px-4 py-4 transition-colors ${
                        selectedId === job.id ? "bg-brand/10" : "hover:bg-surface-2/60"
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="font-body text-[11px] text-fog truncate mt-1">{job.original_filename}</div>
                        </div>
                        <span className={`shrink-0 rounded-full px-2 py-1 font-display text-[9px] uppercase ${meta.cls}`}>
                          {meta.label}
                        </span>
                      </div>
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className={`rounded-full px-3 py-1 font-display text-[10px] uppercase ${status.cls}`}>
                        {status.label}
                      </span>
                    ...
                    <p className="font-body text-sm text-fog mt-1">{selectedJob.original_filename}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {report?.image_url && (
                      <a
              ...
              <div className="grid grid-cols-1 xl:grid-cols-[1.05fr_0.95fr] gap-5">
                <div className="rounded-2xl border border-stroke bg-surface p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-display font-semibold uppercase text-sm text-ink">Concept 2D</h3>
                  </div>
                  {concept?.image_url ? (
                    <img src={assetUrl(concept.image_url)} alt="Concept 2D" className="w-full rounded-xl border border-stroke bg-bg object-contain max-h-[460px]" />
                  ) : (
                  ...
                  {concept?.text_content && <p className="font-body text-xs text-fog mt-3">{concept.text_content}</p>}
                </div>

                <div className="rounded-2xl border border-stroke bg-surface p-5 space-y-4">
                  <div className="flex items-center gap-2">
                    <Brain className="w-5 h-5 text-brand" />
                    <h3 className="font-display font-semibold uppercase text-sm text-ink">Analisi architettonica</h3>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl border border-stroke bg-bg px-4 py-3">
                      <div className="font-display uppercase text-[10px] text-fog">Qualita lettura</div>
                      <div className="font-display font-bold text-sm text-ink uppercase">{qualityLabel(analysisJson.confidence)}</div>
                    </div>
                    <div className="rounded-xl border border-stroke bg-bg px-4 py-3">
                      <div className="font-display uppercase text-[10px] text-fog">Tipo</div>
                      <div className="font-display font-bold text-sm text-ink uppercase">{analysisJson.plan_type_detected || "-"}</div>
                    </div>
                  </div>
                  {analysisJson.dynamic_disclaimer && (
                    <div className="rounded-xl border border-warning/40 bg-warning/10 p-4">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="font-body text-xs leading-relaxed text-fog">{analysisJson.dynamic_disclaimer}</p>
                      </div>
                    </div>
                  )}
                  <div>
                    <p className="font-display uppercase text-xs text-ink mb-2">Ambienti rilevati</p>
                    <div className="space-y-2">
                      {(analysisJson.detected_rooms || []).slice(0, 5).map((room, index) => (
                        <div key={`${room.name}-${index}`} className="rounded-xl border border-stroke bg-bg p-3">
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-display uppercase text-xs text-ink">{room.name}</span>
                            <span className="font-body text-xs text-brand">{qualityLabel(room.confidence)}</span>
                          </div>
                          <p className="font-body text-xs text-fog mt-1">{room.evidence}</p>
                        </div>
                      ))}
                      {(analysisJson.detected_rooms || []).length === 0 && (
                        <p className="font-body text-sm text-fog">Nessun ambiente rilevato con qualita sufficiente.</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                <div className="rounded-2xl border border-stroke bg-surface p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-display font-semibold uppercase text-sm text-ink">Top-down</h3>
                  </div>
                  {topdown?.image_url ? (
                    <img src={assetUrl(topdown.image_url)} alt="Top-down" className="w-full rounded-xl border border-stroke bg-bg object-contain max-h-80" />
                  ) : (
                    <div className="aspect-video rounded-xl border border-stroke bg-bg grid place-items-center text-fog text-sm">
                ...
                <div className="rounded-2xl border border-stroke bg-surface p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-display font-semibold uppercase text-sm text-ink">Render ambienti</h3>
                  </div>
                  {renders.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {renders.slice(0, 4).map((render) => (
                        <div key={render.id} className="rounded-xl border border-stroke bg-bg overflow-hidden">
                          <img src={assetUrl(render.image_url)} alt={render.room_name || "Render"} className="w-full aspect-video object-cover" />
                          <div className="px-3 py-2 font-display uppercase text-[10px] text-brand">{render.room_name}</div>
                        </div>
                      ))}
                    </div>
                  ) : (
