...
export default function Packages() {
  ...
  return (
    <section
      id="soluzioni"
      className="py-24 bg-surface px-6 relative overflow-hidden"
    >
      <div className="absolute inset-0 blueprint-grid opacity-[0.03] pointer-events-none" />
      <div className="relative max-w-7xl mx-auto">
        <p className="font-display font-semibold uppercase tracking-[0.3em] text-xs text-brand mb-3 text-center">
          Le tre soluzioni GB
        </p>
        ...
        <p className="font-body text-fog text-center max-w-2xl mx-auto mb-14">
          Dalla ristrutturazione essenziale al chiavi in mano di lusso. Tocca
          una soluzione per leggere tutti i dettagli del capitolato.
        </p>
          ...
          {PACKAGES.map((p) => (
            <div key={p.key} className="flex flex-col">
              <Tilt3D max={9} radius="rounded-3xl">
                ...
                >
                  {p.badge && (
                    <span className="absolute top-4 right-4 z-10 gold-gradient text-bg font-display font-bold uppercase text-[10px] px-3 py-1 rounded-full">
                      {p.badge}
                    </span>
                  )}
              ...
              <div className="mt-5 text-center">
                <h3
                  className={`font-display font-bold uppercase text-lg ${p.accent}`}
                >
                  {p.name}
                </h3>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        ...
        >
          ...
          {selected && (
            <>
              <img
                src={PROPOSAL_POSTERS[selected.key]}
                alt={selected.name}
